...
it('can add the star name and star symbol properly', async () => {
  ...
  let instance = await StarNotary.deployed();
  assert.equal(await instance.name(), 'YvesStarNotary');
  assert.equal(await instance.symbol(), 'YSN');
});
...
it('lets 2 users exchange stars', async () => {
  ...
  await instance.createStar('another awesome star', starId2, { from: user2 });
...
it('lets a user transfer a star', async () => {
  ...
  let user2 = accounts[2];
  let starId = 8;
  await instance.createStar('star to be transfered', starId, { from: user1 });
...
it('lookUptokenIdToStarInfo test', async () => {
  ...
  let user1 = accounts[1];
  let starId = 9;
  await instance.createStar('star to be looked up', starId, { from: user1 });
  assert.equal(
    await instance.lookUptokenIdToStarInfo(starId),
